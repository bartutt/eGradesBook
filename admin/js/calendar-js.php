...
const Calendar = (id) => ({ 
                    ...
                    let frgEvent = document.createRange().createContextualFragment(`
                        <div class = "event-container">
                            <form action = "<?php $_SERVER['REQUEST_URI']?>" method = "post">
                                <input type = "hidden" name = "value[]" value = "${ev.ev_id}">
                                <input type = "hidden" name = "action" value = "delete_event">
                                <button class="btn btn-danger rounded-0 border-0 event-remove" type="submit"><i class="fas fa-times"></i></button>
                            </form>
                            <div data-toggle="tooltip" data-html="true" title="${ev.desc}" time="${ev.time}" class="event ${ev.cls}">
                                <a href = "details_class.php?class_id=${ev.class_id}">${ev.class_name}</a>
                            </div>
                        </div>
                    `);
    ...
    render(y, m) {
        ...
        if (isNaN(y) && isNaN(this.y)) {
            ...
            this.y = y><?php echo $cal['min'] ?> && y< <?php echo $cal['max'] ?> ? y : moment().year(); // years start-end
        }
